...
from ..utils.eval_old import evaluate
...
def main(args=None):
    ...
    model = retinanet.retinanet_bbox(inputs=inputs, num_classes=72, backbone=resnet)
    model.load_weights(args.model)
    print("args_model:{}".format(args.model))
    ...
    all_detections_with_labels = evaluate(
        generator,
        model,
    ...
    # print(
    #     'all_detections_with_labels:{}\nshape:{}'.format(all_detections_with_labels, all_detections_with_labels.shape)
    # )
    if not os.path.exists('detection.h5'):
        with h5py.File('detection.h5') as f:
            f['dets_labels'] = all_detections_with_labels
    # print evaluation
    # for label, average_precision in average_precisions.items():
    #     print(generator.label_to_name(label), '{:.4f}'.format(average_precision))
    #
    # seenMAP = sum(average_precisions.values()[:72]) / 72
    # unseenMAP = sum(average_precisions.values()[72:]) / 8
    # HMMAP = (2 * seenMAP * unseenMAP) / (seenMAP + unseenMAP)
    # seenRE = sum(average_recalls.values()[:72]) / 72
    # unseenRE = sum(average_recalls.values()[72:]) / 8
